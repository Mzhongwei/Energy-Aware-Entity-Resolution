    ...
    while not stop_requested:
        ...
        if ready is None:
            if stop_requested:
                return
            raise TimeoutError(
                f"[candidate_enumeration] startup timed out after {startup_timeout}s waiting for {INPUT_BUFFER}"
            )
        ...
        if cg_feature is None:
            ...
            print("[candidate_enumeration] worker completed after upstream EOS", file=sys.stderr)
            return
        if not cg_feature:
